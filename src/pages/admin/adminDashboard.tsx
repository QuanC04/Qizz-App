  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const myForms = data.filter(
        (form: any) => form.createdBy === user?.email
      );
      ...
      <ul className="  sm:grid sm:grid-cols-3 grid grid-cols-2">
        {filteredForms.map((form) => (
          <li
            onClick={() => {
              if (!openMenuId) {
                navigate({ to: `/form/edit/${form.formId}` });
              }
            }}
            key={form.formId}
            className="m-5 shadow-xl sm:h-[30vh] h-[20vh] flex flex-col items-start p-2 rounded-2xl  relative cursor-pointer">
            <h2 className="text-2xl pt-3 pr-5 ">{form.title?.titleText}</h2>

            <button
              onClick={(e) => {
                e.preventDefault(); // Ngăn nhảy Link
                e.stopPropagation(); // Giữ menu hoạt động
              ...
              <div
                ...
                <button
                  onClick={async () => {
                    ...
                    setForms((prev) =>
                      prev.filter((f) => f.formId !== form.formId)
                    );
                    setOpenMenuId(null);
                  }}
